...
export default class SubmissionPanel extends BaseUI {
    ...
    DetectionNet() {
        ...
        let data = JSON.stringify(
            GameData.getInstance().createCourseware_content()
        );
    ...
    //添加答案信息
    AddNet(gameDataJson) {
        let data = { title_id: NetWork.title_id, courseware_content: gameDataJson, is_result: ConstValue.IsAnswer ? 1 : 0, is_lavel: ConstValue.IsLevel ? 1 : 0 };
    ...
    ModifyNet(gameDataJson) {
        let jsonData = { courseware_id: NetWork.courseware_id, courseware_content: gameDataJson, is_result: ConstValue.IsAnswer ? 1 : 0, is_lavel: ConstValue.IsLevel ? 1 : 0 };
